...
function CategoryList() {
    ...
    return (<div className="w-full bg-white mt-4 pt-3">
        ...
        <div className="flex justify-center flex-wrap gap-2 pb-3">
            {
                categories.map((category, index) => {
                    return (
                        <a href={category.url}>
                            <div key={index} className="border-2 border-[#ee4d2d] text-gray-600 rounded-lg p-1 hover:bg-[#ee4d2d] cursor-pointer">
                                {category.name}
                            </div>
                        </a>)
                })
